  ...
  <el-card>
    ...
    <el-table :data="mode" style="width: 100%">
      ...
      <el-table-column prop="url" label="图片" align="center">
        <template #default="scoped">
          <img :src="scoped.row.url" style="width: 40px; height: 40px" />
        </template>
      </el-table-column>
    ...
    >
      <el-form :model="form" :rules="rules" ref="submit">
        ...
        <el-form-item label="参数描述" prop="desc" style="margin-left: 10px">
...
let del = (e: any) => {
  console.log(e);
  api
    .delParams({ parentId: e.parentId, attrId: e._id })
    .then((res: any) => {
      console.log(res);
      if (res.code === 200) {
        ElMessage.success(res.msg);
        getParams(e.parentId);
      }
    })
    .catch((err) => {
      console.log(err);
